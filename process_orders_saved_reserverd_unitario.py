...
def update_stock_reservado(product_id, quantity):
		try:
			data = {
			'model': "product.product",
			'domain': json.dumps([['id', '=', product_id]]),
...
#------ Paso 01. Obtiene Ordenes guardadas de Meli.
def get_result_orders(): 
	try:
		...
		query='SELECT user_id, car_id, order_id FROM notify_meli WHERE saved LIKE "2021-05-28%";' # Ordenes del dia
...
#------ Paso 02. De la Orden recuperada de Carrito recupera todos los detalles de la orden desde Meli 
...
#------ Paso 03. De la Orden Normal o Simple leida se recupera todos los detalles de la orden desde Meli 	
...
#------ Paso 04. Procesa los Carritos de Compra de las Ordenes recuperadas
...
#------ Paso 05. Procesa Ordenes Normales independientes que no están en un carrito.
...
#----Paso 06. De los datos recuperados de Meli Mapearlos vs los campos de odoo, formar el Json de para crear el Registro en odoo
#--- Siempre y cuando la orden no exista, si existe actualizar los datos de: 
#--- Se llama al ĺa función para actulizar las piezas reservadas del día (vendidas)
#--- Fin de la funcionaliad del Módulo.
def make_map_meli_odoo(meli_data):
	'''
	-Funcion que crea el Mapeo entre el JSON de Meli y el JSON de Odoo 
	-Creación del Presupuesto SO den Odoo sino esta ya creado en Odoo
	...
	try:
		...
		for productos in lista_productos:
			...
			print ('LINEA  PEDIDO:',product_id, product_uom_qty)
		...
		if existe_orden:
			...
			for productos in lista_productos:
				if meli_data['logistic_type']!='fulfillment':
					product_id =productos[2]['product_id']
					product_uom_qty = productos[2]['product_uom_qty']
					resultado_reservado = update_stock_reservado(product_id, product_uom_qty)
					print ('RESERVADO:'+str(meli_data['date_created'])+'|'+str( meli_data['id_order'])+'|'+str(product_id)+'|'+str(resultado_reservado[0]['default_code'])+'|'+str(product_uom_qty))
					#logging.info()
			#-------------Fin Reserva---------------------------------------------------------------------------------------
