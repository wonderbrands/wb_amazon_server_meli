...
def webhook_venti(product_id, piezas_pedido):
	try:
		...
		if stock_markets_actual==0:
			stock_markets_nuevo = stock_real - piezas_pedido
		else:
		...
		print ('ODOO|stock_exclusivas:', stock_exclusivas, ', stock_urrea:', stock_urrea)

		stock_mercadolibre =  stock_markets_nuevo + stock_exclusivas  + stock_urrea
		if stock_mercadolibre < 0:
			stock_mercadolibre=0

		stock_linio = stock_markets_nuevo + stock_exclusivas
		if stock_linio < 0:
			stock_linio=0

		stock_amazon =  stock_markets_nuevo + stock_exclusivas + stock_urrea
		if stock_amazon < 0:
			stock_amazon = 0

		stock_prestashop =  stock_markets_nuevo + stock_exclusivas + stock_urrea
		if stock_prestashop < 0:
			stock_prestashop = 0

		stock_walmart =  stock_markets_nuevo + stock_exclusivas
		if stock_walmart < 0:
			stock_walmart = 0

		stock_claroshop =  stock_markets_nuevo + stock_exclusivas + stock_urrea
...
def make_map_meli_odoo(meli_data):
	try:
		...
		print 'list EN MAPEO DE CADA ITEM DE LA ORDEN:', list_items
		i=0
