	...
	try:
		...
		if cantidad_productos>0:
			...
			if 'urrea' in categ_id or 'lock' in categ_id or 'surtek' in categ_id :
				stock_neto = stock_urrea - stock_reservado
			else:
...
def get_id_odoo_product_kit(seller_sku, title, id_order, logistic_type):
	try:
		...
		for sub_product in sub_product_line_ids:
			...
			print ('item_selled KIT : ',item_selled)
...
def make_map_meli_odoo(meli_data):
	...
	-Creación del Presupuesto SO den Odoo si no esta ya creado en Odoo
	-Incrementa la reserva de los Productos *
	-Autoconfirma los pedidos si cumplen las siguiente validaciones
	  1. Stock Suficiente
	  2. Precio Correcto con el procentaje de descuento de +- 15% 
	  3. Tiene la etiqueta para imprimir
