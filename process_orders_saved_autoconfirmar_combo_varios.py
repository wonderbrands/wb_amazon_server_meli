...
def get_order_meli(seller_id, order_id, access_token):
	try:
		...
		if order_exist>0:
			...
			feedback = order.get('feedback').get('sale')
			...
			for item in order_items:
				...
				logging.info('ORDER ID: ' + str(id_order) +', SELLER SKU:'+ str(seller_sku) +', MLM: ' + str(mlm) + ', LOGISTICA MELI: ' + str(logistic_type) )
				...
				if 'COMBO' in seller_sku:
					...
					if (porcentaje_descuento < 40.0) and (quantity <= stock_neto_odoo) and (etiqueta_impresion==True):
					...
					if (porcentaje_descuento < 40.00) and (quantity <= stock_neto_odoo) and (etiqueta_impresion==True):
						print ('SI AUTOCONFIRMAR')
						autoconfirmar = True
					else:
						print ('NO AUTOCONFIRMAR')
	...
	try:
		...
		for pedido in pedidos_de_carrito:
			...
			if feedback==None:
				...
				fulfilled = feedback.get('fulfilled')
	...
	try:
		...
		if existe_orden:
			...
			if meli_data['logistic_type']=='fulfillment':                        #'Meli Fulfilment'
				...
				state_so = 'draft'
				print('tipo lista_productos[0]', type(lista_productos[0]) )

				#----------------------Para reservar--------------------------------------------------------------------------
				print('Extracion:',type(lista_productos[0]) )
				productos=[]
				if len(lista_productos)==1 and  str(type(lista_productos[0])) =="<class 'tuple'>":
...
if __name__ == '__main__':
	...
	#ordenes = [('25523702', '2000002694021027', '4774401544'), ('160190870', '2000002685114836', '4739093725'),]
